...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public void configureGlobalSecurity(AuthenticationManagerBuilder auth) throws Exception {
		...
		if (authentication.equalsIgnoreCase(AuthenticationConstants.DB_AUTH)) {
			...
			//auth.authenticationProvider(customDBAuthenticationProvider);
	 ...
	 * "/list","/home","/userpermission","/usermenuright","/editProduct")
	 ...
	 * tokenRepository)
	 * .tokenValiditySeconds(86400).and().exceptionHandling().accessDeniedPage(
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/", "/list", "/home", "/userpermission", "/usermenuright", "/editProduct")
				...
				.access("hasRole('ADMIN') or hasRole('DBA')").and().formLogin().loginPage("/login").permitAll().and()
	...
	public DefaultJaasAuthenticationProvider dbAuthenticationProvider() {
		...
		appConfigEntries[0] = new AppConfigurationEntry("com.capgemini.webapp.security.authentication.CustomJaasLoginModule",
	...
	public DefaultJaasAuthenticationProvider ldapAuthenticationProvider() {
		...
		/*
		 * Map<String, String> options = new HashMap<String, String>();
		 * options.put(AuthenticationConstants.DEBUG,
		 * env.getRequiredProperty(AuthenticationConstants.DEBUG));
		 */

		Map<String, String> options = new HashMap<String, String>();
		options.put(AuthenticationConstants.USER_PROVIDER,
				env.getRequiredProperty(AuthenticationConstants.USER_PROVIDER));
		...
		AppConfigurationEntry appConfigEntries[] = new AppConfigurationEntry[1];
		appConfigEntries[0] = new AppConfigurationEntry(
				env.getRequiredProperty(AuthenticationConstants.LDAP_LOGIN_MODULE),
				javax.security.auth.login.AppConfigurationEntry.LoginModuleControlFlag.REQUIRED, options);
		// appConfigEntries[0] = new
		// AppConfigurationEntry("com.capgemini.pocwebapp.spring.config.security.Login",
